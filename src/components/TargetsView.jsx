            ...
            {traders.map((trader) => (
              <tr key={trader.id}>
                ...
                {RATE_COLS.map((column) => (
                  <td key={column.key}>
                    <input className="cell" type="number" onFocus={(event) => event.target.select()} value={trader.rates[column.key] ?? 0} onChange={(event) => onRate(trader.id, column.key, Number(event.target.value) || 0)} />
