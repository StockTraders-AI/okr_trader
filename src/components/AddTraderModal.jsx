...
export default function AddTraderModal({ traders, onClose, onCreate }) {
  ...
  async function createTrader() {
    ...
    if (!raw) {
      setError("Vui lòng nhập userName/account.");
      return;
    }

    if (traders.some((trader) => sameAccount(trader.user, raw) || sameAccount(trader.name, raw))) {
      setError("Account đã tồn tại trong danh sách trader.");
      return;
    }
    ...
    try {
      const accountInfo = await checkAccount(raw);

      if (traders.some((trader) => sameAccount(trader.user, accountInfo.userName) || sameAccount(trader.name, accountInfo.userName))) {
        setError("Account đã tồn tại trong danh sách trader.");
        return;
      }

      onCreate(accountInfo);
    } catch (err) {
      setError(err?.message || "Không kiểm tra được account.");
    } finally {
      setLoading(false);
    }
  }
    ...
    <div className="overlay" onClick={(event) => event.target.classList.contains("overlay") && onClose()}>
      <div className="modal">
        <div className="mhead">
          ...
          <div><div className="mtitle">Thêm trader mới</div><div className="msub">Tạo tài khoản trader bằng userName đã active</div></div>
        </div>
        ...
        <div style={{ display: "flex", gap: 10, marginTop: 6 }}>
          <button className="btn-ghost" style={{ flex: 1 }} onClick={onClose} disabled={loading} type="button">Huỷ</button>
          <button className="btn-primary" style={{ flex: 1.4 }} onClick={createTrader} disabled={loading} type="button">{loading ? "Đang kiểm tra..." : "Tạo trader"}</button>
        </div>
        <div className="note" style={{ margin: "16px 0 0", fontSize: 11, lineHeight: 1.6 }}>Chỉ tạo trader khi userName/account đã active trên StockTraders. Tên trader sẽ lấy theo userName API trả về.</div>
      </div>
    </div>
  );
}
